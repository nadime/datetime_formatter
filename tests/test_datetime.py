...
def test_string_to_datetime_time():
    assert _string_to_datetime("08:30:15") == _combine_time(
        datetime.time(8, 30, 15)
    )
    assert _string_to_datetime("14:21:27.13") == _combine_time(
        datetime.time(14, 21, 27, 130000)
    )
    assert _string_to_datetime("19:47:38.131010") == _combine_time(
        datetime.time(19, 47, 38, 131010)
    )
    assert _string_to_datetime("19:47") == _combine_time(
        datetime.time(19, 47, 0)
    )
    assert _string_to_datetime("8:47") == _combine_time(
        datetime.time(8, 47, 0)
    )
    assert _string_to_datetime("13:15:00.999999") == _combine_time(
        datetime.time(13, 15, 0, 999999)
    )
    with pytest.raises(ValueError):
...
def test_datetime_class():
    ...
    assert _DateTime(datetime.datetime(2005, 3, 1)).dt == datetime.datetime(
        2005, 3, 1
    )
    assert _DateTime(datetime.date(2005, 3, 1)).dt == datetime.datetime(
        2005, 3, 1
    )
    assert _DateTime(
        datetime.time(18, 30, 10)
    ).dt == datetime.datetime.combine(
        datetime.datetime.today(), datetime.time(18, 30, 10)
    )
...
def test_datetime_translate_dates():
    ...
    assert s20210528.translate("business_days", 1, holidays.US()) == mkdt(
        2021, 6, 1
    )
    ...
    # sat + 1 biz week = mon
    assert s20210306.translate("business_weeks", 1, None) == mkdt(2021, 3, 15)
    assert _DateTime(20210313).translate("business_weeks", -1, None) == mkdt(
        2021, 3, 5
    )

    # months
    assert s20211125.translate("months", 1, holidays.US()) == mkdt(
        2021, 12, 26
    )
    assert s20211125.translate("months", 1, None) == mkdt(2021, 12, 25)

    # make sure month jump into a holiday at end of month and beginning
    # of months works appropriately
    ...
    assert s20211031.translate("months", 1, test_holidays) == mkdt(
        2021, 11, 29
    )
    assert s20211201.translate("months", 1, test_holidays) == mkdt(2022, 1, 2)
    assert s20220101.translate("months", -1, test_holidays) == mkdt(
        2021, 12, 2
    )
    # landing on weekends translates back/fwd business days
    assert _DateTime(20210206).translate("business_months", 1, None) == mkdt(
        2021, 3, 8
    )
    assert _DateTime(20210406).translate("business_months", -1, None) == mkdt(
        2021, 3, 5
    )
    ...
    assert s20221130.translate("years", -1, test_holidays) == mkdt(
        2021, 11, 29
    )
    assert s20201130.translate("years", 1, test_holidays) == mkdt(2021, 11, 29)
    # landing on weekends translates back/fwd business days
    assert _DateTime(20200306).translate("business_years", 1, None) == mkdt(
        2021, 3, 8
    )
    assert _DateTime(20220306).translate("business_years", -1, None) == mkdt(
        2021, 3, 5
    )

    # make sure if we iterate by months (or years) and pass through a holiday,
    # we don't change our landing date
    assert s20211030.translate("months", 2, test_holidays) == mkdt(
        2021, 12, 30
    )
    assert s20211030.translate("months", 1, test_holidays) == mkdt(
        2021, 11, 29
    )
    ...
    def test_datetime_translate_times():
        ...
        assert s20210301.translate("hours", -5) == s20210301.dt - rd(hours=5)
        assert s20210301.translate("seconds", 5) == s20210301.dt + rd(
            seconds=5
        )
        assert s20210301.translate("seconds", -5) == s20210301.dt - rd(
            seconds=5
        )
        assert s20210301.translate("microseconds", 5) == s20210301.dt + rd(
            microseconds=5
        )
        assert s20210301.translate("microseconds", -5) == s20210301.dt - rd(
            microseconds=5
        ...
        assert s20210301.translate("hours", 13) == datetime.datetime(
            2021, 3, 2, 1, 0, 0
        )
